            ...
            if (gBattleMons[battler].status2 & STATUS2_UPROAR)
            {
                for (gBattlerAttacker = 0; gBattlerAttacker < gBattlersCount; gBattlerAttacker++)
                {
                    if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
                     && (B_UPROAR_IGNORE_SOUNDPROOF >= GEN_5 || (GetBattlerAbility(gBattlerAttacker) != ABILITY_SOUNDPROOF && GetBattlerAbility(gBattlerAttacker) != ABILITY_CACOPHONY)))
                    {
...
static void CancellerFrozen(u32 *effect)
{
    ...
    {
		if (gBattleWeather & B_WEATHER_SUN)									//ZETA- Defrost in Sun
                {
				...
				else if (gBattleWeather & B_WEATHER_HAIL)					//ZETA- Never defrost in Hail
				{
					if (!gBattleStruct->isAtkCancelerForCalledMove 
					&& (gBattleMons[gBattlerAttacker].status1 & STATUS1_FREEZE))
					{
						if (!RandomPercentage(RNG_FROZEN_MOVE, 75))			//ZETA- 75% Chance to immobilise
						{
							gProtectStructs[gBattlerAttacker].nonVolatileStatusImmobility = TRUE;
				...
				{
					if (!RandomPercentage(RNG_FROZEN, 10))					//ZETA- 10% Thaw chance
					{
						if (!gBattleStruct->isAtkCancelerForCalledMove 
						&& (gBattleMons[gBattlerAttacker].status1 & STATUS1_FREEZE) 
						&& !RandomPercentage(RNG_FROZEN_MOVE, 75))			//ZETA- 75% Chance to immobilise
						{
							gProtectStructs[gBattlerAttacker].nonVolatileStatusImmobility = TRUE;
							gBattlescriptCurrInstr = BattleScript_MoveUsedIsFrozen;
							gHitMarker |= (HITMARKER_NO_ATTACKSTRING | HITMARKER_UNABLE_TO_USE_MOVE);
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_SWITCH_IN_TERRAIN:   // terrain starting from overworld weather
        ...
        else if ((GetCurrentWeather() == WEATHER_FOG_DIAGONAL) //removed config check. FOG_DIAGONAL now sets misty terrain, FOG_HORIZONTAL sets fog weather
    ...
    case ABILITYEFFECT_SWITCH_IN_WEATHER:
        ...
        if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED))
        {
            ...
            case WEATHER_SNOW:
                ...
                break;
            //case WEATHER_FOG_DIAGONAL: - ZETA set diagonal fog to misty terrain
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case HOLD_EFFECT_DEEP_SEA_TOOTH:
        if (gBattleMons[battlerAtk].species == (SPECIES_CLAMPERL || SPECIES_HUNTAIL) && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
...
static inline u32 CalcDefenseStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    case HOLD_EFFECT_DEEP_SEA_SCALE:
        if (gBattleMons[battlerDef].species == (SPECIES_CLAMPERL || SPECIES_GOREBYSS) && !usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
	...
	//added def boost for hail
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE) && ((IsBattlerWeatherAffected(battlerDef, B_WEATHER_SNOW) && usesDefStat) || (IsBattlerWeatherAffected(battlerDef, B_WEATHER_HAIL) && usesDefStat)))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static uq4_12_t GetWeatherDamageModifier(struct DamageCalculationData *damageCalcData, u32 holdEffectAtk, u32 holdEffectDef, u32 weather)
{
	...
	if (weather & B_WEATHER_FOG)
    {
        if (moveType != TYPE_FLYING && moveType != TYPE_BUG)
            return UQ_4_12(1.0);
        return (moveType == TYPE_FLYING) ? UQ_4_12(0.5) : UQ_4_12(1.5);
    }
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
		...
		|| (abilityDef == ABILITY_DESERT_SPIRIT)
		|| (abilityDef == ABILITY_DAUNTING_WINGS)
...
void ActivateMegaEvolution(u32 battler)
{
	...
	AdjustFriendship(&gPlayerParty[gBattlerPartyIndexes[battler]], FRIENDSHIP_EVENT_MEGA_EVOLVE);
    if (GetBattleFormChangeTargetSpecies(battler, FORM_CHANGE_BATTLE_MEGA_EVOLUTION_MOVE) != gBattleMons[battler].species)
...
bool8 CanMonParticipateInSkyBattle(struct Pokemon *mon)
{
    ...
    bool8 hasLevitateAbility = gSpeciesInfo[species].abilities[monAbilityNum] == (ABILITY_LEVITATE || ABILITY_DESERT_SPIRIT || ABILITY_DAUNTING_WINGS || ABILITY_WIND_CHIME);
