...
bool8 PartyHasMonWithSludgeSurf(void)
{
    ...
    if (!TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING))
    {
		...
		for (u8 j = 0; j < MAX_MON_MOVES; j++)
			{
				u16 moveId = GetMonData(&gPlayerParty[i], MON_DATA_MOVE1 + j, NULL);
				if (!GetMonData(&gPlayerParty[i], MON_DATA_IS_EGG) && gMovesInfo[moveId].fieldMoveFlags & IS_FIELD_MOVE_SLUDGE)
                return TRUE;
			}
        }
    }
...
bool8 IsPlayerFacingFreshWater(void)
{
    ...
    if (GetCollisionAtCoords(playerObjEvent, x, y, playerObjEvent->facingDirection) == COLLISION_ELEVATION_MISMATCH
     && PlayerGetElevation() == 3
     && MetatileBehavior_IsFreshWater(MapGridGetMetatileBehaviorAt(x, y)))
        return TRUE;
    else
        return FALSE;
}
...
bool8 IsPlayerFacingSludge(void)
{
    ...
    if (GetCollisionAtCoords(playerObjEvent, x, y, playerObjEvent->facingDirection) == COLLISION_ELEVATION_MISMATCH
     && PlayerGetElevation() == 3
     && MetatileBehavior_IsSludge(MapGridGetMetatileBehaviorAt(x, y)))
        return TRUE;
    else
        return FALSE;
}
...
static bool32 CheckTileQualification(s16 tile[], s16 player[], u32 facingDirection, struct ObjectEvent* objectEvent, bool32 isTileLand[], u32 direction)
{
    ...
    if (IsPlayerHere(tile[AXIS_X], tile[AXIS_Y], player[AXIS_X], player[AXIS_Y]))
        ...
        return FALSE;
    else if (MetatileBehavior_IsSludge(MapGridGetMetatileBehaviorAt(tile[AXIS_X], tile[AXIS_Y])))
        return FALSE;
