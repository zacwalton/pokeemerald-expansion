...
void SwitchInClearSetData(u32 battler)
{
    ...
    if (effect == EFFECT_BATON_PASS)
    {
        ...
        gStatuses4[battler] &= (STATUS4_MUD_SPORT | STATUS4_WATER_SPORT | STATUS4_APOTROPAISM | STATUS4_INFINITE_CONFUSION);
        for (i = 0; i < gBattlersCount; i++)
        {
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, enum ItemHoldEffect holdEffect)
{
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE || holdEffect == HOLD_EFFECT_POWER_ITEM)
        ...
        speed *= 2;
    else if (holdEffect == HOLD_EFFECT_SWEET_FRUIT && gBattleMons[battler].species == SPECIES_TROPIUS && (gBattleWeather & B_WEATHER_SUN))
        speed *= 2;
